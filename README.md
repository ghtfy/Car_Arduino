...
이번 프로젝트는 온전히 팀원들과의 소통과 자료조사만을 통해 고민하고 개발하였습니다.

* ** 영상처리의 전문성**: 외부 자료에 의존해 독학하며 기초 체력의 중요성을 절실히 느꼈습니다. 이번 경험을 발판 삼아, 더 깊이 있는 영상 처리 전공 지식을 쌓아 알고리즘의 원리를 정확히 이해하고 개선해보고 싶습니다.
* ** 효율적인 코드 (최적화)**: 라즈베리파이라는 한정된 자원 위에서 코드가 무거워질 때마다 성능의 한계를 경험했습니다. 단순히 작동하는걸 넘어, 하드웨어의 최적화가 필요함을 느꼈습니다.
* * **문서화 부족**: 체계적인 문서화의 부족으로 무엇이 오류인지 실시간 피드백 및 개선에 한계를 나타냈습니다.
